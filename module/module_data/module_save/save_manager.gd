...
## 存档数据
const GameStateData: Script = preload("res://addons/godot_system/module/module_data/module_save/game_state_data.gd")

## 项目设置路径常量
const SETTING_SAVE_DIR: String = "godot_system/module_save/save_directory"
const SETTING_SAVE_EXT: String = "godot_system/module_save/save_extension"
const SETTING_MAX_AUTO_SAVES: String = "godot_system/module_save/max_auto_saves"
const SETTING_AUTO_SAVE_INTERVAL: String = "godot_system/module_save/auto_save_interval"
const SETTING_AUTO_SAVE_ENABLED: String = "godot_system/module_save/auto_save_enabled"
...
func create_save(save_name: String, callback: Callable = func(_success: bool): pass) -> void:
	...
	# 收集所有可序列化组件的数据
	var serializable_nodes = System.get_tree().get_nodes_in_group(SerializableComponent.GROUP_NAME)
	var serialized_data = {}
		...
		func(success: bool, result: Variant):
			if success:
				...
				for node_path in serialized_data.keys():
					var node = System.get_node_or_null(node_path)
					if node is SerializableComponent:
